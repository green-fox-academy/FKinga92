...
def ingredient_list():
    max = 0
    length_of_second = int(len("needs cooling"))
    length_of_third = int(len("in stock"))
    for i in ingredients:
        if len(i["name"]) > max:
            max = len(i["name"])
    linedeco = ("+" + "-" * (max + 2) + "+" + "-" * (length_of_second + 2) +
		       "+" + "-" * (length_of_third + 2) + "+")
    print(linedeco)
    print("| " + "{:<{}}".format("Ingredient", max) + " | " +
		  "{:<{}}".format("Needs cooling", length_of_second) + " | " +
		  "{:<{}}".format("In stock", length_of_third) + " |")
    print(linedeco)
    for i in ingredients:
        print("| " + "{:<{}}".format(i["name"], max) + " | " +
			  "{:<{}}".format(str(i["needs_cooling"]).replace("True", "Yes").replace("False", "No"), length_of_second) +
			  " | " + "{:<{}}".format(str(i["in_stock"]).replace("0", "-"), length_of_third) + " |")
    print(linedeco)
