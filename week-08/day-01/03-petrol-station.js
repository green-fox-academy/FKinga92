...
let car = {
  ...
  refill: function(amount) {
    let neededAmount = this.petrolCapacity - this.petrolLevel;
    if (amount >= neededAmount) {
      this.petrolLevel +=  neededAmount;
      return neededAmount;
    }
    else {
...
let station = {
  ...
  provide: function(car) {
    let consumedAmount = car.refill(this.petrolStorage);
    this.petrolStorage -= consumedAmount;
  }
};
